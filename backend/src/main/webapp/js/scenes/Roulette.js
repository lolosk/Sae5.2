...
// CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE CREATE
  ...
  create(){
    ...
    const timerMarginRight = 270; // avant 16 → plus grand = plus à gauche

    // Affichage du temps en haut-droite
    this.timerTxt = this.add.text(W - timerMarginRight, 40, 'Temps: 30s', {
    ...
    // Coupe le timer de manche et les sons de tic-tac
    _stopTimerAndSounds(){
      // Stopper l'event de timer s'il existe
      if (this.timerEvent) {
        this.timerEvent.remove(false); // false = ne pas rappeler immédiatement le callback
        this.timerEvent = null;
      }

      // Arrêter les sons de tic-tac
      if (this.sfx) {
        if (this.sfx.slow && this.sfx.slow.isPlaying) this.sfx.slow.stop();
        if (this.sfx.fast && this.sfx.fast.isPlaying) this.sfx.fast.stop();
      }

      // Optionnel : remettre la phase à "none"
      ...
      _makeHomeButton(){
        ...
        this.homeBtn
          ...
          .on('pointerup',   () => {
            this.homeBtn.clearTint();
            this._stopTimerAndSounds();   // <--- coupe tic-tac + timer
